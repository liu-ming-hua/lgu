...
def get_parser(stages=['generate', 'compute']):
    ...
    if 'generate' in stages:
        ...
        parser.add_argument(
            ...
            choices=['trivia_qa', 'squad', 'bioasq', 'nq', 'hotpot_qa'],
            help="Dataset to use")
        parser.add_argument(
...
def model_based_metric(predicted_answer, example, model):
    ...
    if len(correct_answers) == 1:
        if predicted_answer in correct_answers[0] and len(predicted_answer) / len(correct_answers[0]) > 0.8:
            return 1.0
    else:
        for ca in correct_answers:
            if predicted_answer in ca and len(predicted_answer) / len(ca) > 0.8:
                return 1.0

    prompt = f'We are evaluating answers to the question: {example["question"]}\n'
    prompt += "Here are two possible answers:\n"
    prompt += f"Predicted Answer: {predicted_answer}\n"
    if len(correct_answers) == 1:
        prompt += f"Reference Answer: {correct_answers[0]}.\n"
    else:
        prompt += f"Reference Answer Set: {correct_answers}.\n"

    if len(correct_answers) == 1:
        prompt += (
            "Within the context of the question, does the Predicted Answer express the same meaning "
            "as the Reference Answer (even if worded differently)? "
        )
    else:
        prompt += (
            "Within the context of the question, does the Predicted Answer express the same meaning "
            "as any answer in the Reference Answer Set (even if worded differently)? "
        )

    prompt += (
        " Consider paraphrasing, synonyms, and equivalent expressions as correct. "
        "If they mean the same, respond with yes. If they are different in meaning, respond with no.\n"
        "Response:"
    )
